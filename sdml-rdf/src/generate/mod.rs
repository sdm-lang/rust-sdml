...
fn identifier_reference_to_url(
    ...
    Ok(match name_reference {
        IdentifierReference::Identifier(ident) => ctx.base_uri.join(ident.as_ref()),
        IdentifierReference::QualifiedIdentifier(ident) => {
            let other_base_uri = cache
                .module_name_to_uri(ident.module())
                .expect(&format!("imported module not loaded: {name_reference}"));
            other_base_uri.join(ident.member().as_ref())
        }
    }
    .expect("Url parse error"))
}
