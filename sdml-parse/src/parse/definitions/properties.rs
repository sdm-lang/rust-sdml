...
use sdml_core::model::definitions::{PropertyBody, PropertyDef, PropertyRole};
use sdml_core::model::members::{
    ByReferenceMemberDef, ByValueMemberDef, HasCardinality, IdentityMemberDef,
};
...
use sdml_core::syntax::{
    ...
    NODE_KIND_ROLE_BY_REFERENCE, NODE_KIND_ROLE_BY_VALUE,
};
            ...
            if node.is_named() {
                match node.kind() {
                    ...
                    NODE_KIND_ROLE_BY_REFERENCE => {
                        body.add_to_roles(parse_role_by_reference(context, &mut node.walk())?);
                    }
                    NODE_KIND_ROLE_BY_VALUE => {
                        body.add_to_roles(parse_role_by_value(context, &mut node.walk())?);
                    }
...
fn parse_identity_role<'a>(
    ...
    let mut def = IdentityMemberDef::new(type_reference).with_source_span(node.into());
...
fn parse_role_by_reference<'a>(
    ...
    context.check_if_error(&child, RULE_NAME)?;
    let type_reference = parse_type_reference(context, &mut child.walk())?;
    let mut def = ByReferenceMemberDef::new(type_reference).with_source_span(node.into());
...
fn parse_role_by_value<'a>(
    context: &mut ParseContext<'a>,
    cursor: &mut TreeCursor<'a>,
) -> Result<PropertyRole, Error> {
    let node = cursor.node();
    rule_fn!("role_by_value", node);
    ...
    context.check_if_error(&child, RULE_NAME)?;
    let type_reference = parse_type_reference(context, &mut child.walk())?;
    let mut def = ByValueMemberDef::new(type_reference).with_source_span(node.into());
